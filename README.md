...
This interactively creates a `.env` file with literal-safe quoted values, a bcrypt password hash, JWT secret, IP hashing salt, and the public HTTPS URL used by the deployment. Review the generated file before starting the service. Keep the quotes generated by setup because the bcrypt hash contains `$` characters.
...
The setup prompt requires the public HTTPS origin. The container listens on port `3000` and stores the embedded SQLite database in the persistent `ding-data` Docker volume. No separate database service is required. For a live deployment, keep the port bound to localhost and put Caddy, Nginx, or another HTTPS reverse proxy in front of it; see [Deployment](docs/DEPLOYMENT.md).
...
Read the [Ding documentation](docs/README.md) for detailed setup, configuration, API reference, and deployment guides.

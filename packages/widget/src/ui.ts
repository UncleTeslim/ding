...
export function injectStyles(config: WidgetConfig) {
  ...
  style.textContent = `
    ...
    .ding-badge { position:absolute; top:-5px; right:-5px; min-width:19px; height:19px; padding:0 5px; border-radius:999px; background:var(--ding-badge); color:#fff; font-size:12px; line-height:19px; text-align:center; font-weight:700; animation:ding-badge-pop 300ms ease-out; }
    @keyframes ding-badge-pop { 0% { transform:scale(0); } 60% { transform:scale(1.2); } 100% { transform:scale(1); } }
    .ding-backdrop { position:fixed; inset:0; background:rgba(0,0,0,0.2); z-index:calc(var(--ding-z) - 1); opacity:0; pointer-events:none; transition:opacity 200ms ease; }
